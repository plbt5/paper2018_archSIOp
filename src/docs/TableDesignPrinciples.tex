...
\quad When it is reasonable to expect that the software agent will be engaged in collaboration or otherwise will interoperate with (an)other software agent(s), it is the responsibility of the DSP to disclose the semantic model of its DM, and the ontological commitment that it applies, to the extent necessary for comprehension.
\end{mmdp}
&
\begin{description}[labelwidth=3.7cm,leftmargin=3.7cm+1ex,nosep,topsep=2ex,labelsep=1ex,font=\bfseries]
  \item[Type of information:] Business, Data;
...
\begin{enumerate}[left=6pt, nosep]
  ...
  \item The user of the software agent has a natural, business-controlled perspective on the DoA. This particular perspective is mirrored by the design of the software agent in the way how the entities of interest are carved out from the DoA. This includes the ontological commitment that applies, the used terminology and how data from these entities are to be processed, collaboratively known as the semantic model;
  ...
  \item On specifying semantics, Grice’s maxims on communication, and particularly on serving the quantity of communication, represent the natural constraints to respect. This implies that a DSP is to disclose the semantic model about the exchanged data, including the ontological commitment that is applied;
...
&
\begin{enumerate}[left=10pt, nosep]
  \item The semantic model, which provides the semantic meaning to the data, is only dependent on the agent’s own perspective on the application domain, and can therefore be designed without any specific demands on interoperability with other software agents;
...
\quad Semantic compatibility is a precursor to semantic interoperability. In order to establish the level of semantic compatibility between collaborating software agents, both agents must make their ontological commitment underlying their domain models, explicit.\end{mmdp} 
&
\begin{description}[labelwidth=3.7cm,leftmargin=3.7cm+1ex,nosep,topsep=2ex,labelsep=1ex,font=\bfseries]
\item[Type of information:] Business, Data;
...
\begin{enumerate}[left=6pt, nosep]
  ...
  \item An ontological commitment represents a (mostly philosophical) stance on fundamental categories, their scope and principles, which are foundational to semantics \cite[sec.3.1]{Brandt2021a};
  ...
  \item Keeping an ontological commitment implicit implies to commit to a subjective and vague categorisation,  scope and principles, the application of which will probably result in an inconsistent and inaccurate if not obscure and ambiguous semantic model. The explicit use of an ontological commitment significantly improves the quality of the semantic model;
  \item When their underlying ontological commitments are in conflict, achieving semantic interoperability between two semantic theories becomes harder if not impossible;
  \item Semantic compatibility is a precursor to the emergence of sIOP; 
...
&
\begin{enumerate}[left=10pt, nosep]
  \item Semantic compatibility represents a similarity range, influenced by the following parameters:
  \begin{enumerate}
    \item Per agent: (i) the level of ontological rigor (clarity and formality of its categorisation, scope and principles) of the OC; (ii) the relationship that exists between the OC and the semantic model, either implicit and associative or explicit as generalisation-specialisation; 
    \item Per DSP/DSC pair: the existence of an alignment between both OCs and their level of overlap, which is maximal with the application of the same OC for both semantic models.
  \end{enumerate}
  \item The higher the compatibility between the semantic models, the more efficient the software agents can realise sIOP;
  \item The lower the compatibility between the semantic models, the more issues between the semantic models of the agents must be resolved, if at all possible, in order to achieve sIOP;
...
\end{description} \\
\begin{enumerate}[left=6pt, nosep]
  \item The quintesential of exchanging data about the overlapping DoA is to exchange the SoA that apply;
  ...
  \item The meaning that the agent's conceptualisation intends to convey is represented by a semantic model. Such representation (of the SoA) can only approximate the intended meaning. Those SoAs are allowed by the semantic model and, hence, represent the semantic model's allowed meaning. The difference between the allowed and intended meaning equals invalid meaning;
...
\begin{enumerate}[left=10pt, nosep]
  ...
  \end{enumerate}
  \item An overlap between the DSP's and DSC's semantic models now also implies a maximal overlap in what is intended to convey with a minimal overlap of invalid meaning; 
...
\quad When it is reasonable to expect that the software agent will be engaged in collaboration or otherwise will interoperate with (an)other software agent(s), the agent should provide the capability to converse about the purpose of the communication.
\end{mmdp}
&
\begin{description}[labelwidth=3.7cm,leftmargin=3.7cm+1ex,nosep,topsep=2ex,labelsep=1ex,font=\bfseries]
  \item[Type of information:] Business;
